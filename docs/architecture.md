...
`apply` closes the same loop in the opposite direction so a device can materialize cloud-authored memory instead of mirroring it. The authority that assigned a record its id stays the authority: `apply` writes the ids it is given and mints none. A staged batch of complete commits is applied in one transaction; records are ordered by dependency inside each commit, deduplicated against a `(record_kind, record_id, payload_hash)` ledger, and rejected when they reference an unknown source, evidence, or claim, contradict a stored payload, or revive a tombstone. Evidence locators are stored exactly as exported, so an applied claim remains citable back to the revision it came from. Applied records enter the local commit feed, which keeps `export` complete on a replica and keeps deletion propagation, projection repair, and retrieval working over foreign records without a separate mirror.
