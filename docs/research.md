...
| --- | --- | --- |
| [Omi](https://github.com/BasedHardware/omi) | Its canonical memory work separates short-, long-, and archive-tier items; retains source evidence; versions ledger mutations; invalidates superseded facts; and purges vectors and graph citations when sources are retracted. | **Borrow:** canonical tiers (`short_term`, `long_term`, `archive`), processing states (`pending`, `processed`, `blocked`), a legal-state matrix, source-backed claims, deterministic commits, projection-repair outbox, and fail-closed deletion. **Avoid:** coupling the neutral crate to Firebase, Pinecone, Neo4j, or Omi policy. |
...
| [MIRIX](https://github.com/Mirix-AI/MIRIX) | Separates core, episodic, semantic, procedural, resource, and knowledge-vault memory; ingests visual, voice, and text activity through a multi-agent personal-assistant service. | **Borrow:** explicit memory categories and source-aware multimodal capture. **Avoid:** continuous capture, multi-agent dispatch, PostgreSQL, and service lifecycle in the neutral crate. |
| [Microsoft GraphRAG](https://github.com/microsoft/graphrag) | Uses LLM extraction to form a knowledge graph, clusters it into communities, and generates hierarchical summaries; its maintainers explicitly call indexing expensive. | **Borrow:** a graph only as a measured multi-hop or corpus-level retrieval projection. **Defer:** LLM graph construction and community summaries until they beat cited lexical+dense retrieval without deletion or scope regressions. |
...
| [OptMem](https://github.com/VictorTaelin/OptMem) | Uses an append-only fixed-width log and a rebuildable binary summary tree, with explicit foreground compaction and exact-log recall. | **Borrow:** a bounded session wake and explicit compaction lifecycle. **Avoid:** a parallel authority, background compaction, and uncited summaries; the repository has no declared license, so no code is copied. |
...
| Canonical tiers | Claims live in `short_term`, `long_term`, or `archive`. Only `short_term` and `long_term` processed claims are retrievable; `archive` is excluded from live retrieval but kept in history. |
| Legal state | Every claim write is checked against an explicit legal-state matrix of tier × status × processing_state. |
| Projection repair | Tier changes, supersession, retraction, and deletion enqueue an idempotent outbox. `repair` drains it and never mutates durable records. |
