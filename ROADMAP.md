...
1. **Versioned schema migrations:** replace create-if-missing setup with ordered, rollback-safe upgrades and fixtures from every released schema.
2. **Scoped export and import:** round-trip one tenant/person with evidence, history, tombstones, and projection metadata without leaking another scope.
3. **Frozen retrieval benchmark:** check FTS-only, dense-only, and hybrid retrieval against corrections, deletion, multilingual paraphrases, abstention, and tenant isolation.
4. **Evidence-backed graph expansion:** add typed adjacency only if the frozen benchmark proves a repeatable gain over hybrid retrieval.
