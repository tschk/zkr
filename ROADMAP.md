...
- Scoped export and idempotent, caller-identified import (`apply`) that round-trips one tenant/person.
- Native OpenClaw and Hermes adapters over the same CLI contract.

## Next, in order

1. **Frozen retrieval benchmark:** check FTS-only, dense-only, and hybrid retrieval against corrections, deletion, multilingual paraphrases, abstention, and tenant isolation.
2. **Evidence-backed graph expansion:** add typed adjacency only if the frozen benchmark proves a repeatable gain over hybrid retrieval.
