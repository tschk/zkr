...
fn repair_projections_deletes_stale_embeddings_when_target_modified() {
    ...
    let raw = db.remember(remember_raw("a", "sam", "A quiet desk")).unwrap();
    ...
    let processed_at: i64 = db.connection.query_row(
...
fn repair_projections_deletes_embeddings_when_target_deleted() {
    ...
    db.connection.execute("PRAGMA foreign_keys = OFF", []).unwrap();
    db.connection.execute("DELETE FROM sources WHERE id = ?1", [&raw.source_id.0]).unwrap();
    db.connection.execute("PRAGMA foreign_keys = ON", []).unwrap();
