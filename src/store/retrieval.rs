...
impl MemoryDb {
    ...
    pub(super) fn retrieval_targets_for_embeddings_bulk(
    ...
    ) -> Result<HashMap<(String, String), Vec<RetrievalTarget>>> {
        if targets.is_empty() {
            return Ok(HashMap::new());
        }
        for (kind, _) in targets {
            if !matches!(kind.as_str(), "source" | "evidence" | "claim") {
                return Err(Error::Invalid(
        ...
        let inputs_json =
            serde_json::to_string(targets).map_err(|error| Error::Invalid(error.to_string()))?;
        let sql = "
        WITH inputs AS (
            SELECT
        ...
        WHERE EXISTS (
            SELECT 1 FROM evidence live_e
            WHERE live_e.source_id = s.id AND live_e.tenant_id = ?1 AND live_e.person_id = ?2 AND live_e.deleted_at IS NULL
        )
        AND NOT EXISTS (
            SELECT 1
            FROM evidence e
        ...
        let mut results: HashMap<(String, String), Vec<RetrievalTarget>> = HashMap::new();
        for row in rows {
