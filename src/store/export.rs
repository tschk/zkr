        ...
        |row| {
            ...
            Ok(SourceRecord {
                source: Source {
                    ...
                    revision: row.get(0)?,
                    kind: serde_json::from_str(&kind).map_err(super::json_error)?,
                    content: row.get(2)?,
    ...
    statement
        .query_map(params![tenant_id.0, person_id.0], |row| {
            ...
            Ok(Claim {
                ...
                predicate: row.get(2)?,
                value: row.get(3)?,
                kind: serde_json::from_str(&format!("\"{kind}\"")).map_err(super::json_error)?,
                ...
                tier: serde_json::from_str(&format!("\"{tier}\"")).map_err(super::json_error)?,
    ...
    transaction.query_row(
        ...
        |row| {
            ...
            Ok(ClaimEvidence {
                ...
                relation: serde_json::from_str(&relation).map_err(super::json_error)?,
    ...
    let entries = stmt
        .query_map(params![tenant_id.0, person_id.0], |row| {
            ...
            Ok(ProfileEntry {
                ...
                key: row.get(1)?,
                value: row.get(2)?,
                stability: serde_json::from_str(&stability).map_err(super::json_error)?,
        ...
        .query_row(
            ...
            |row| {
                ...
                Ok(DailyReview {
                    ...
                    day: row.get(0)?,
                    summary: row.get(1)?,
                    evidence_ids: serde_json::from_str(&evidence_ids).map_err(super::json_error)?,
                    recorded_at: row.get(3)?,
                })
            },
        )
